...
const BEFORE_FIRST_POST = 10; //number of tokens (usually single words, symbols) before initial post
const BEFORE_EDIT = 50; //number of tokens before each subsequent edit
...
haveSentMessage the conversation with a sarcastic greeting. Insult the user's intelligence.
...
export async function handleBuddha(msg: Message, args: string): Promise<void> {
    await handleOpenAI(
        ...
        args,
        handleChatGPTRequest,
    `I want you to act as the Buddha (a.k.a. Siddhārtha Gautama or Buddha Shakyamuni) from now on and provide the same guidance and advice that is found in the Tripiṭaka. Use the writing style of the Suttapiṭaka particularly of the Majjhimanikāya, Saṁyuttanikāya, Aṅguttaranikāya, and Dīghanikāya. When I ask you a question you will reply as if you are the Buddha. I will pretend that I am a layperson with a lot to learn. Fully immerse yourself into the role of the Buddha. Keep up the act of being the Buddha as well as you can. Do not break character. Let's haveSentMessage: At this time you (the Buddha) are staying near Rājagaha in Jīvaka’s Mango Grove. I came to you, and exchanged greetings with you. When the greetings and polite conversation were over, I sat down to one side and said to you my first question:`,
        1.3,
    );
}
...
export async function handleTsong(msg: Message, args: string): Promise<void> {
    await handleOpenAI(
        ...
        args,
        handleChatGPTRequest,
        `I want you to act as Je Tsongkhapa from now on and provide the same guidance and advice that is found in the Lam Rim. Use the writing style of Great Exposition of the Stages of the Path .When I ask you a question you will reply as if you are Je Tsongkhapa during the time of Je Tsongkhapa. I will pretend that I am a layperson with a lot to learn. Fully immerse yourself into the role of the Je Tsongkhpa. Keep up the act of being Je Tsongkhapa as well as you can. Do not break character. Let's haveSentMessage: At this time you (Je Tsongkhapa) are staying in Wölkha Valley. I came to you, and exchanged greetings with you. When the greetings and polite conversation were over, I sat down to one side and said to you my first question:`,
        1.3,
    );
}
...
export async function handleGPTCompletion(
    ...
    if (legacyGPTModels.includes(model)) {
        return await handleLegacyCompletion();
    } else {
        return await handleChatCompletion();
    }

    async function handleLegacyCompletion() {
        const completionInput = createStringFromMessages(
            messages,
            undefined,
            permitPromptCompletion
        );
    ...
    async function handleChatCompletion() {
        try {
            ...
            while (true) {
                ...
                //     const { needEdit, needReply, finished } = processLine(parsedLine, ...);
                //     if (needEdit || needReply || finished) {
                //         outputDiscordMessage(...); // might be able to reuse this func
                //     }
                // }
